...
const toWeVoice = (value) => clean(value)
  ...
  .replace(/امتیاز دهید/g, 'امتیاز بدهیم')
  .replace(/فهرست کنید/g, 'فهرست کنیم')
  .replace(/ثبت کنید/g, 'ثبت کنیم')
...
const sessionOpenings = {
  m2: `خوش آمدید. جلسهٔ قبل، روزمان را با سه نفر در سپهر شروع کردیم و دیدیم هوش مصنوعی زمانی معنا پیدا می‌کند که یک اصطکاک واقعی را کم کند. امروز همان سه نفر برگشته‌اند؛ این بار هرکدام یک پیشنهاد دارند.
...
const audiencePrompts = {
  ...
  m2: {
    s01: ['اگر همین حالا مجبور به انتخاب باشید، بودجه را به پیشنهاد سارا، رضا یا مریم می‌دهید؟', 'با نشان‌دادن دست یک رأی اولیه بگیرید و نتیجه را نگه دارید؛ در اسلاید پایانی همین رأی را تکرار می‌کنیم.'],
    ...
    s15: ['حالا دوباره رأی بدهید: سارا، رضا یا مریم؟ آیا انتخاب شما نسبت به ابتدای جلسه تغییر کرده است؟', 'نتیجه را با رأی اول مقایسه کنید. سپس انتخاب سپهر، یعنی تیکت هوشمند رضا، را با چهار دلیل «حجم، دامنه، بازبینی و مالک روشن» جمع‌بندی کنید.']
  },
  m3: {
...
function factualContext(slide) {
  ...
  if (!note) return '';
  const imperativeTokens = [' کن', ' نکن', 'بگو', 'بپرس', 'بخواه', 'بخوان', 'بگیر', 'بگذار', 'بساز', 'ببین', 'بده', 'نده', 'ببر', 'بیاور', 'بزن', 'ببند', 'کنید', 'دهید', 'بخواهید', 'نشان بده', 'نگه دار', 'پرهیز کن', 'یادآوری کن', 'دعوت کن', 'اشاره کن'];
  return note
    .split(/(?<=[.!؟؛])\s+/)
...
function practicalExample(slide, session) {
  ...
  if (session.id === 'm2' && /سه پیشنهاد، یک بودجه/.test(key)) return 'سه گزینه عمداً متفاوت‌اند: پیشنهاد سارا به دانش معتبر و کنترل دادهٔ کارکنان نیاز دارد؛ پیشنهاد رضا به تیکتینگ و پایگاه دانش وصل می‌شود؛ پیشنهاد مریم با متن جلسه و یک بازبین انسانی شروع می‌شود. پس یک فناوری مشترک، سه طراحی متفاوت می‌سازد.';
  if (session.id === 'm2' && /انتخاب انجام شد/.test(key)) return 'انتخاب تیکت هوشمند به معنی بی‌ارزش‌بودن دو پیشنهاد دیگر نیست. سپهر گزینه‌ای را جلو می‌اندازد که در ۹۰ روز می‌تواند آن را محدود اجرا کند، نتیجه را بسنجد و در صورت خطا بدون آسیب جدی متوقف کند.';
